...
Outputs in the folder: `openlap_results.png` (each channel on its own axes, grouped: speed, curvature, acceleration, driver inputs, attitude, plus GGV and map), `hud.html` (original dark grid: header, follow-cam stage, side DRIVER / G-G / TIRES / MAP, bottom telemetry), `hud_frame.png`, `channels.csv`. The stage uses a world-aligned asphalt ribbon (~110 m field of view, dashed centerline, throttle/brake trail) with a UBCO-liveried FSAE car on the road and in the tires card. The TIRES card updates FL/FR/RL/RR Fz, power, and energy every frame. DRIVER BPS/TPS bars and percentages update with the lap. Channels are still QSS estimates (not a live 6DOF NLP). Use `--synthetic` if `libfastestlapc` is not on `LD_LIBRARY_PATH`. The uploaded Sketchfab `Carmodel.gltf` is missing its `untitled.bin` mesh; the HUD car is a Three.js FSAE with white nose / navy “1”, navy sidepods and flame graphics, and “UBC OKANAGAN RACING” on the rear wing.
